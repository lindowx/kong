...
<error>
  ...
  local function get_response_type(content_header)
    ...
    if content_header ~= nil then
      ...
      for _, value in ipairs(accept_values) do
        ...
        for _, entry in ipairs(mimetype_values) do
          ...
          if m then
            ...
            if m then
              ...
              m = ngx.re.match(entry, [[^\s*level=(\d)\s*$]], "ajoi")
              if m then
                level = tonumber(m[1])
              end
            end
        ...
        if level <= hi_level and quality > hi_quality then
          if find(name, CONTENT_TYPE_JSON, nil, true) == 1 then
          ...
          elseif find(name, CONTENT_TYPE_PLAIN, nil, true) == 1 then
  ...
  --
  -- ```lua
  -- return kong.response.exit(500, "Error", {["Content-Type"] = "text/html"})
  -- ```
  --
  -- The first argument `status` will set the status code of the response that
  ...
  function _RESPONSE.error(status, message, headers)
    ...
    local content_type = get_response_type(content_type_header)

    headers[CONTENT_TYPE_NAME] = CONTENT_TYPE_HEADERS[content_type]
